...
Given a number A, find the smallest possible value of B+C, if B*C = A. Here A, B, and C must all be positive integers. Its okay to use brute force by checking every possible value of B and C. You dont need to handle inputs larger than six digits. Post the return value for A = 345678 along with your solution.

For instance, given A = 12345 you should return 838. Heres why. There are four different ways to represent 12345 as the product of two positive integers:
...
In this case, you can assume youre given a list of primes instead of the number itself. (To check your solution, the output for this input ends in 22.)
'''

// finds the lowest sum of multiples of given value
...
int main(void) {
	...
	// loops until all divisible numbers are found
	while (quotient != i) {
		
		// finds evenly divisible numbers
		if ( (val % i) == 0) {
			quotient = val / i;
			if (sum < i + quotient)
				break;
			else
				sum = i + quotient;		// sum will always decrease as i increasess
			if (DEBUG)
				printf("%d = %d + %d\t sum = %d\n", val, i, quotient, sum);
		}
		i++;
